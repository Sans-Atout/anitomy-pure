...
impl SubToken {
    ...
    pub(crate) fn new(v: &str) -> SubToken {
        SubToken {
            value: v.to_string(),
    ...
    pub(crate) fn category(&mut self, c: SubTokenCategory) -> &mut SubToken {
        self.category = c;
        self
    }

    /// Returns `true` if this subtoken's category matches `c`.
    pub(crate) fn is_category(&self, c: SubTokenCategory) -> bool {
        self.category == c
    }
...
pub(crate) enum SubTokenCategory {
